...
strategy or clone an existing strategy. Allo provides a BaseStrategy that all
strategies will inherit from. For more information, see the [Writing a Custom
Strategy](/writing-a-custom-strategy.md) page. Strategies must be created prior
to creating a new pool and will be called by the `Allo.sol` contract.
...
determine what makes a wallet eligibile to be an allocator. Allocation can be as
